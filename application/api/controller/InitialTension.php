...
use app\common\model\InitialTension as InitialModel;
use mrmiao\encryption\RSACrypt;
use think\Db;


//初张拉通知单
class InitialTension extends ApiBase
{

    //初张拉通知单(获取参数)
    public function getTension(RSACrypt $crypt,InitialModel $tension)
    {
        try {
            $post_data = $crypt->request();

            //验证参数
            $result = $this->validate($post_data, 'InitialTension.get');
            if (true !== $result)
    ...
    //发布初张拉通知单
    public function addTension(RSACrypt $crypt,InitialModel $tension,TaskProcess $taskProcess,TaskFlow $taskFlow)
    {
        try {
            $post_data = $crypt->request();
            ...
            if ($post_data['type'] == 1){//执行添加
                ...
                //验证参数
                $result = $this->validate($post_data, 'InitialTension.add');
                if (true !== $result)
                    return $crypt->response(['code' => 400, 'message' => $result],true);

                //当前工序状态
                $process_status = $taskProcess->where(['task_id'=>$post_data['task_id'],'process_id'=>19])
                ...
                //验证参数
                $result = $this->validate($post_data, 'InitialTension.edit');
                if (true !== $result)
                    return $crypt->response(['code' => 400, 'message' => $result],true);
    ...
    //查看初张拉通知单
    public function lookTension(RSACrypt $crypt,InitialModel $tension)
    {
        try {
            $post_data = $crypt->request();

            //验证参数
            $result = $this->validate($post_data, 'InitialTension.look');
            if (true !== $result)
                return $crypt->response(['code' => 400, 'message' => $result],true);
